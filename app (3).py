...
st.markdown("""
<style>
@import url('[fonts.googleapis.com](https://fonts.googleapis.com/css2?family=DM+Serif+Display:ital@0;1&family=DM+Sans:wght@300;400;500&display=swap)');
...
DEFAULT_IMG = "[images.unsplash.com](https://images.unsplash.com/photo-1519915028121-7d3463d20b13?w=400&q=80)"
...
SNS_FEEDS = [
    {
        ...
        "url": "[instagram.com](https://www.instagram.com/explore/tags/%EC%84%B1%EC%88%98%EB%B2%A0%EC%9D%B4%EC%BB%A4%EB%A6%AC/)",
        "tag": "#성수베이커리 #크루아상맛집",
        "likes": "8,241"
    },
    {
        ...
        "url": "[search.naver.com](https://search.naver.com/search.naver?query=%EC%97%B0%EB%82%A8%EB%8F%99+%EB%94%94%EC%A0%80%ED%8A%B8+%EC%B9%B4%ED%8E%98)",
        "tag": "#연남동카페 #데이트코스",
        "likes": "5,103"
    },
    {
        ...
        "url": "[cafe.naver.com](https://cafe.naver.com/ArticleSearchList.nhn?search.query=%EA%B0%95%EB%82%A8+%EB%B9%B5%EC%A7%91)",
        "tag": "#강남빵집 #파티스리",
        "likes": "3,782"
    },
    {
        ...
        "url": "[instagram.com](https://www.instagram.com/explore/tags/%EC%86%8C%EA%B8%88%EB%B9%B5/)",
        "tag": "#소금빵 #홍대맛집",
        "likes": "12,558"
    },
    {
        ...
        "url": "[search.naver.com](https://search.naver.com/search.naver?query=%EB%B6%81%EC%B4%8C+%EC%A0%84%ED%86%B5%EC%B9%B4%ED%8E%98)",
        "tag": "#북촌카페 #전통디저트",
        "likes": "4,430"
    },
    {
        ...
        "url": "[instagram.com](https://www.instagram.com/explore/tags/%EC%84%9C%EC%9A%B8%EC%BC%80%EC%9D%B4%ED%81%AC/)",
        "tag": "#서울케이크 #케이크맛집",
        "likes": "9,871"
    },
    {
        ...
        "url": "[cafe.naver.com](https://cafe.naver.com/ArticleSearchList.nhn?search.query=%ED%95%9C%EB%82%A8%EB%8F%99+%EC%B9%B4%ED%8E%98)",
        "tag": "#한남동카페 #이태원맛집",
        "likes": "2,990"
    },
    {
        ...
        "url": "[instagram.com](https://www.instagram.com/explore/tags/%EB%A7%88%EC%B9%B4%EB%A1%B1%EB%A7%9B%EC%A7%91/)",
        "tag": "#마카롱투어 #서울마카롱",
        "likes": "6,204"
    },
]
...
def load_data():
    ...
    # dessert.csv 파일을 찾도록 수정
    candidates = [
        os.path.join(BASE_DIR, "dessert.csv"),  # 메인 파일명
        os.path.join(BASE_DIR, "seoul_dessert_spots.csv"),  # 대체 파일명
        "dessert.csv",
    ...
    if "naver_url" not in df.columns:
        df["naver_url"] = df["name"].apply(
            lambda n: f"[map.naver.com](https://map.naver.com/v5/search/{urllib.parse.quote(str(n))})"
        )
    ...
    with col_map:
        ...
        if len(filtered):
            ...
            for _, row in filtered.iterrows():
                ...
                naver = row.get("naver_url", f"[map.naver.com](https://map.naver.com/v5/search/{urllib.parse.quote(row)['name'])}")
        ...
        with st.form("add_place_form", clear_on_submit=True):
            ...
            if submitted:
                if not new_name or not new_addr:
                    ...
                    naver_link = f"[map.naver.com](https://map.naver.com/v5/search/{urllib.parse.quote(new_name) + ' ' + new_dist)}"
                    new_row = {
    ...
    if not display_rows:
        ...
        for i, row in enumerate(display_rows[:63]):
            with cols[i % 3]:
                ...
                naver  = row.get("naver_url") or f"[map.naver.com](https://map.naver.com/v5/search/{urllib.parse.quote(str(row.get()'name','')))}"
...
with tab3:
    ...
    q_cols = st.columns(4)
    quick_links = [
        ("📸 인스타 #성수베이커리", "[instagram.com](https://www.instagram.com/explore/tags/%EC%84%B1%EC%88%98%EB%B2%A0%EC%9D%B4%EC%BB%A4%EB%A6%AC/)"),
        ("📝 네이버 서울카페 리뷰", "[search.naver.com](https://search.naver.com/search.naver?query=%EC%84%9C%EC%9A%B8+%EB%94%94%EC%A0%80%ED%8A%B8+%EC%B9%B4%ED%8E%98+%EC%B6%94%EC%B2%9C)"),
        ("☕ 다이닝코드 서울빵집", "[diningcode.com](https://www.diningcode.com/list.dc?query=%EC%84%9C%EC%9A%B8+%EB%B2%A0%EC%9D%B4%EC%BB%A4%EB%A6%AC)"),
        ("🏅 망고플레이트 카페", "[mangoplate.com](https://www.mangoplate.com/search/%EC%84%9C%EC%9A%B8%20%EB%94%94%EC%A0%80%ED%8A%B8)"),
    ]
    ...
    for idx, (orig_i, post) in enumerate(posts):
        ...
        with st.container():
            ...
            with col_info:
                ...
                place_link = ""
                if post.get("place"):
                    nurl = f"[map.naver.com](https://map.naver.com/v5/search/{urllib.parse.quote(post)['place'])}"
    ...
    with col_b:
        ...
        if len(filtered):
            ...
            for _, g in gems.iterrows():
                ...
                naver = g.get("naver_url", f"[map.naver.com](https://map.naver.com/v5/search/{urllib.parse.quote(str(g)['name']))}")
                st.markdown(f"""
                <a href="{naver}" target="_blank" style="text-decoration:none;">
